...
!> @brief This routine computes the min-norm solution of a problem

!> This routine computes the min-norm solution of an underdetermined system
...
!! @param[in]           b the RHSs. A 2D array of leading dimension qrm_mat%m. On output
!!                        it will contain b
!! @param[out]          x the solution, i.e., Q*R'\b
!!
subroutine _qrm_min_norm2d(qrm_mat, b, x, info)
  ...
  ! error management
  integer                          :: err
  character(len=*), parameter      :: name='qrm_min_norm'
  ...
  if(qrm_mat%n .lt. qrm_mat%m) then
     err = 31
  ...
  call qrm_analyse_async(qrm_dscr, qrm_mat, _qrm_transp, err)
  __QRM_INFO_CHECK(err, name, 'qrm_analyse_async', 9998)
  call qrm_barrier(qrm_dscr)
  call qrm_factorize_async(qrm_dscr, qrm_mat, _qrm_transp, err)
  ...
  do i=1, (nrhs-1)/nb+1
     ...
     call qrm_solve_async(qrm_dscr, qrm_mat, _qrm_transp, b_rhs(i), x_rhs(i), err)
     __QRM_INFO_CHECK(err, name, 'qrm_solve_async', 9998)
  call qrm_barrier(qrm_dscr)
     call qrm_apply_async(qrm_dscr, qrm_mat, 'n', x_rhs(i), err)
...
!!                        the result of the analysis and factorization phases will
!!                        be stored in the adata and fdata fields, respectively.
!! @param[in]           b the RHSs. A 1D array of leading dimension qrm_mat%m. On output
!!                        it will contain b
!! @param[out]          x the solution, i.e., Q*R'\b
!!
subroutine _qrm_min_norm1d(qrm_mat, b, x, info)
  ...
  use qrm_error_mod
  use _qrm_utils_mod
  use _qrm_methods_mod, savesym => _qrm_min_norm1d
  implicit none
  ...
  ! error management
  integer                         :: err
  character(len=*), parameter     :: name='qrm_min_norm1d'
  ...
  call _qrm_min_norm2d(qrm_mat, pntb, pntx, info)
